...
void dm_rpkgc_ ( int *iflno, int *iiword, int *lendat, 
...
/*---------------------------------------------------------------------*/
	...
	*iret = -35;
         ...
         */
        if  ( *ipktyp == MDGGRB )  {
	    dp_ugrb_  ( ksgrid, kxky, nbits, ref, scale, miss, rdata, iret );
        }
        else if  ( *ipktyp == MDGNMC )  {
    	    dp_unmc_  ( ksgrid, kxky, nbits, ref, scale, rdata, iret );
        }
        else if  ( *ipktyp == MDGDIF )  {
	    dp_udif_  ( ksgrid, kxky, nbits, ref, difmin, scale, 
		       miss, kx, rdata, iret );
        }
        else  {
	    *iret = -31;
