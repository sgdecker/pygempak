...
C               qtype = 'dbTime'
C               qparms = dbmodel
C               CALL ST_NULL ( qtype, qtype, lenq, ier )
C               CALL ST_LCUC (dbdatasrc, src,ier)
C               CALL ST_NULL ( src, src, lenq, ier )
C               CALL ST_NULL ( qparms, qparms, lenq, ier )
C
C* Get the list of available data times.
C 
C               CALL DB_GTIM ( qtype, src, qparms, 
C     +                        timstr, ltimstr, iret )
C               IF ( iret .ne. 0 ) THEN
C                  iret = -17
C                  RETURN
C               END IF
C               ntimex = ltimstr/11
C               CALL ST_CLSL (timstr(:ltimstr), '|', ' ', ntimex,
C     +                       timlist, ntx, iret)
C               IF (ntx .gt. 25 ) THEN
C                  do itim=ntx-24, ntx
C                     dbtimes(itim-(ntx-24)+1)= timlist(itim)
C                  end do
C                  ntime = 25
C               ELSE
C                  do itim=1, ntx
C                    dbtimes(itim)= timlist(itim)
C                  end do
C                  ntime = ntx
C               END IF
C               iret = 0
C               gridtmdb = .false.
C               dbtime = dbtimes(igdtim)
C               igdtim = igdtim + 1
               RETURN
           ELSE
